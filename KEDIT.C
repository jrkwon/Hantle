...
/*
 *  Õeª≥ã° ∂ÖµwµA ¨a∂wñAìe •eÆÅói..
 */
bool hanja_font_exist;                    /* –e∏a ãiç© Ãa∑©∑° π•∏Å–aìe ª° */

bool all_jobs_close;                      /* all jobs close */

/*
 *  Õeª≥µA ª¢∏Û â≈ùeñE •eÆÅói
 */
unsigned char *memory;                   /* Ø°∏b–i òÅ Õeª≥–i †e«q∑Å â∑àe∑i –iîw§hìeîa */
unsigned char *line_start;               /* ¥·ìa –e ∫â∑Å Ø°∏b Õ°∑•»· */
unsigned char *current;                  /* —e∏Å Õeª≥ ∫ó∑• ∂·√° */
unsigned char *last_pos;                 /* §aù° ∑°∏Â∑Å ∂·√° */
unsigned char *b_b_ptr;                  /* ßiú‚∑Å Ø°∏b Õ°∑•»· */
unsigned char *b_k_ptr;                  /* ßiú‚∑Å è{ Õ°∑•»· */
unsigned char *eofm;                     /* Ãa∑©∑Å †aª°†b */
unsigned char work_file[80];             /* ∏b¥Û Ãa∑©∑Å ∑°üq */
unsigned char block_file[80];            /* ßiú‚∑aù° ∏·∏w/∑™¥·µ°ã° Ãa∑©∑Å ∑°üq */
unsigned char write_file[80];            /* ¨Å∑°üq∑aù° Ãa∑©∑Å ∑°üq */
unsigned char help_file[80];             /* ï°∂ë†i Ãa∑©∑Å ∑°üq */
unsigned char line_buffer[MAXLINEBUF+1]; /* —e∏Å Õeª≥ ∫ó∑• ∫â∑Å ∑±Ø° ∏b¥Û ∏w≠° */
...
/* ƒ·¨·üi ∂·¥aúÅù° ∂ëª¢∑© âw∂Å ∑°∏Â∑Å ƒ·¨· ∂·√°àa ª°ãq ƒ·¨·∂·√°•°îa «aîa°e ∑°üi ã°¥‚ */
...
static void disp_coord_status(void)
{
  ...
  sprintf(msg, "%4u∫â%4uƒe", base_y+cury+1, base_x+curx+1);
...
void disp_status(void)
{
  ...
  sprintf(msg, " %s‘≥%4u∫â %3uƒe ‘≥ %s ‘≥ %s ‘≥ %s",
      ...
      (insert_mode) ? "¨s∑≥" : "ÆÅ∏˜",
      (indent_mode) ? "óiµa≥aã°" : "êÅ¥·≥aã°", split_filename(work_file));
    ...
    if(get_filename_to_write(temp_name) != CANCEL_ANS) {
      ...
      if(files(temp_name) != NULL) {
        ...
        if(file_or_dir_exist(temp_name)) {
          if(yesno(" Ãa∑©∑° ∑°£° ∑∂ìeïA, î¸¥· ≥iåa∂a ? ", NO_ANS) == YES_ANS)
...
static int find( void )
{
  ...
  if(success) {
    ...
    if(found <= f_count) {
      errno = (find_flag) ? 39 : 37;  /* î·∑°¨w ¥Ùîa : ãaúÂà· ¥Ùîa */
      errorf();
    }
    found = 0;
    find_flag = NO;
...
void  find_condition(int is_replace)
{
  ...
  if(win_hgets(40, "¿x∑i ¢Ö∏aµi∑e ?", object))  {
    if(is_replace && !win_hgets(40, "§aéâ ¢Ö∏aµi∑e ?", replace_str))
      return;
    strcuttail(object);
    ...
    if(win_hgetdata(40, "¨Â»Ç¨a–w (G B U W N Æï∏a)", "EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE", option)) {
      ...
      if(f_count == 0)
        f_count = 1;
      sense = (strchr(option, 'B')) ? -1 :1 ; /* »q¨Ç §w–∑ */
